...
| tests/operator.txt (milestone 8) | 10 goals on local test pages with the real browser hands + 1.5B model: read prices/policies, open pages, search, add to cart (must ask first), buy (must ask first), sign-in goal (must refuse) | **10/10** | `python3 engine/scripts/score_operator.py` (~12 min on 2 sandbox cores; 9–95 s per goal). Live sites: Etsy founding year and CEO from Wikipedia, "search Wikipedia for dropshipping → first sentence" — all correct |
